...
Requirements 开头说明目标，按需包含背景、范围、用户场景、可观察行为与验收、成功标准、边界情况、关键实体、非目标、假设和来源，不写实现步骤或任务进度；保留会影响业务承诺的技术硬约束。每个可独立失败的验收点说明角色或条件、触发、结果和必要反例；同一 R 下有多个验收点时使用稳定子编号，不按句子机械编号。Design 明确复用与新增边界、接口和数据流、状态与错误、兼容、关键假设及验证，避免把关键选择留给实现阶段；存在真实技术取舍时比较可行方案。生成前分别读取 `templates/feature/requirements.md`、`templates/feature/design.md`；附件按需创建。README 记录当前摘要、入口、审阅状态、下一步和交付状态；规划时快照注明日期，当次会话的执行授权不写入长期需求或设计正文。
