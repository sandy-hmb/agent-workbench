...
书面设计获批后直接在 `plans/implementation.md` 写唯一书面计划草案并从 README 链接，不先展示摘要等待生成确认；只有任务、基线、分支、执行方式或验证方式存在无法从设计、规范和代码事实解决的关键阻塞时才询问。需求仍保持 `planning`。先检查设计是否包含可独立交付的子系统；能独立验收的子系统拆成不同需求或计划。
...
向用户展示实际计划文件、工作类型、基线、候选分支及执行方式，等待明确批准。仅在批准后才去除草案标记、更新 README 为 `development` 并转交 `workspace-execute-plan`。计划只有这一轮针对实际文件和执行条件的批准门禁；创建计划与开始实现之间必须保留它。
