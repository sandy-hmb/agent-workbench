...
用于已获批准的 `development` 需求。新会话先运行 status，并在多需求时显式指定 feature slug；随后每个任务只需一次 `brief <slug> --task T01 --execution --check --json --projection execution`，同时取得任务正文、直接依赖、设计引用、`instructionContext` 和执行决策，并校验文档与规范入口，不默认全文加载历史记录；不得依赖历史对话。轻量改动不使用本 Skill。
...
1. 用 `brief <slug> --task <currentTask.id> --execution --check --json --projection execution` 一次展开当前任务并校验规范入口；退出码非 0 时停止，报告缺失或不安全的规范入口，不开始编辑。不得跳过 `currentTask` 自行选择 `readyTasks` 中的后续任务。
