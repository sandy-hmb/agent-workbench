...
def delivery(root: Path, slug: str, changes: dict, *, expected_revision: str) -> dict:
    ...
    with item_lock(directory):
        ...
        for name, row in changes.get('repositories', {}).items():
            object_fields(row, allowed={'version','submission','deployment','acceptance','evidence','evidenceRefs'}, field=f'$.repositories.{name}')
            if name not in {b['repository'] for b in state['bindings']}:
                raise WorkItemError('DELIVERY_INVALID', '仓库交付归属无效')
            if set(row) - {'version', 'submission', 'deployment', 'acceptance', 'evidence', 'evidenceRefs'}:
                raise WorkItemError('DELIVERY_INVALID', '仓库交付字段无效')
            text(row.get('evidence'), f'$.repositories.{name}.evidence')
            if 'evidenceRefs' in row:
                row['evidenceRefs'] = validate_evidence_refs(root, slug, row['evidenceRefs'], f'$.repositories.{name}.evidenceRefs')
...
def render(root: Path, slug: str) -> dict:
    ...
    def workflow_refs(row):
        refs = row.get('evidenceRefs', []) if isinstance(row, dict) else []
        if not isinstance(refs, list):
            return ''
        return '；'.join(
            f"{one(ref.get('runId'))}/{one(ref.get('stage'))}（{one(ref.get('status', 'unknown'))}）"
            for ref in refs if isinstance(ref, dict) and ref.get('kind') == 'workflow'
        )
