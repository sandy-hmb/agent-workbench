...
from workbench.work_items.query import item_summary  # noqa: E402
...
REMEDIATIONS: dict[str, Remediation] = {
    ...
    "ITEM_METADATA_INVALID": Remediation(
        "manual",
        "对照 templates/item/README.md 核对该需求 README 的状态字段格式（状态、"
        "需求短名、工作分支、基线分支、最后更新）。",
    ),
...
def _check_instruction_layers(
    ...
    if workspace_agents.is_file() and not workspace_agents.is_symlink() and root_agents.is_file():
        ...
        try:
            template = (
                KIT_ROOT / "templates/workspace/AGENTS.md"
...
def _check_item_metadata(workspace: Workspace, findings: list[Finding], item_slug: str | None = None) -> None:
    items = items_root(workspace.root)
    if not _safe_directory(workspace.root, items, required=True):
        return
    try:
        children = [items / item_slug] if item_slug is not None else sorted(items.iterdir())
    except OSError as exc:
        findings.append(
            finding("ERROR", "ITEM_METADATA_INVALID", f"{items}：{exc}")
        )
        return
...
def _check_workflow_artifacts(root: Path, findings: list[Finding]) -> None:
    ...
    if not overlay.is_file() or overlay.is_symlink():
        return
    try:
        from workbench.extensions.runner import status_result
...
def markdown_files(root: Path, item_slug: str | None = None) -> Iterable[Path]:
...
def _check_markdown_links(
    root: Path, findings: list[Finding], workspace: Workspace | None = None, item_slug: str | None = None
) -> None:
    root = root.resolve()
    markdown_roots = (
        root / "docs",
    ...
    for markdown in markdown_files(root, item_slug):
        try:
...
def _check_extensions(root: Path, findings: list[Finding]) -> None:
    try:
        from workbench.extensions.management import extension_findings
...
def audit(
    ...
    if not registry.exists() and not registry.is_symlink():
        ...
        if state_present and (
            ...
            findings.append(
                finding(
                    ...
                    "工作区尚未初始化；先运行 scripts/kit.py setup init plan "
        ...
        _check_markdown_links(root, findings, item_slug=item_slug)
        return findings
    state = state_root(root)
    ...
    if version != VERSION:
        return [finding("ERROR", "WORKSPACE_FORMAT_UNSUPPORTED", "仅支持新版配置；旧数据保持原状，请在新目录初始化")]
    try:
    ...
    _check_item_metadata(workspace, findings, item_slug)
    ...
    _check_clients_and_skills(root, findings)
    _check_extensions(root, findings)
    _check_markdown_links(root, findings, workspace, item_slug)
    if verbose:
