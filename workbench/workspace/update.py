...
from workbench.extensions.model import ExtensionError, load_manifest  # noqa: E402
from workbench.extensions.management import _read_lock, extension_sources  # noqa: E402
from workbench.workspace.model import WorkspaceError  # noqa: E402
from workbench.workspace.paths import extensions_root  # noqa: E402
...
def plan_result(root: Path) -> dict[str, object]:
    ...
    if not blockers:
        result["applyCommand"] = shlex.join(
            [
                "python3", "scripts/kit.py", "update", "apply", "--root", str(root),
...
def apply_result(root: Path, expected_hash: str) -> dict[str, object]:
    ...
    check = subprocess.run(
        [sys.executable, "-B", str(root / "scripts/kit.py"), "doctor", "--root", str(root), "--verbose", "--json"],
        cwd=root,
        capture_output=True,
        text=True,
        encoding="utf-8",
