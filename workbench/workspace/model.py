...
def workspace_schema_version(root: Path) -> int:
    from workbench.workspace.paths import state_root, workspace_file
...
def _validate_pattern(value: object, label: str) -> str:
    ...
    try:
        probe = value.format(owner="owner", type="item", slug="slug")
...
def parse_workspace(
    ...
    *,
    ...
    if (
        ...
        raise WorkspaceError(f"workspace.json version.major 必须为 {VERSION}；请新建工作区")
    context = raw.get("context", {})
    if not isinstance(context, dict):
        raise WorkspaceError("context 必须是 JSON 对象")
    extensions = raw.get("extensions")
    from workbench.extensions.registry import discover_extensions, normalize_workspace_extensions
...
def _render_repository_profile(
    ...
    template_path = KIT_ROOT / "templates" / template_name
    try:
