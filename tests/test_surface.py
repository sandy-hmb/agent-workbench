...
class SurfaceTest(unittest.TestCase):
    ...
    def test_operational_skills_use_registry_and_remote_branch_state(self):
        ...
        self.assertIn("requirements.md", agents)
        self.assertIn("references/api-contract.md", agents)
        self.assertIn("独立附件只展开细节", agents)
        self.assertIn("Design 保留完整方案和 D 决策", agents)
        self.assertIn("plan.md", agents)
        self.assertIn("历史对话", agents)
        self.assertIn("默认使用中文", agents)
        self.assertIn("错误原文", agents)
        self.assertIn("用户可见", agents)
        self.assertIn("结果、下一步和必要决策", agents)
    ...
    def test_feature_stage_templates_and_rules_stay_aligned(self):
        ...
        self.assertIn("[需求](requirements.md)", next_plan)
        self.assertIn("[设计](design.md)", next_plan)
