...
公共工作流行为变化属于标准需求：在 `docs/development/features/<feature-slug>/` 逐阶段记录。先确认需求再创建需求文件，确认方案后创建设计，确认计划与验证策略后创建实施计划并编码；新计划使用 `task-evidence-v2`，验证记录只在实际执行后创建。局部、可定向验证的维护可以走轻量流程。
