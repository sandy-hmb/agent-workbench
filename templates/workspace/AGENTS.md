...
- 展开任务后按 `instructionContext` 读取工作区、目标仓和就近规范入口，再按实际改动类型读取入口明确索引的专项规范；首次编辑对应代码前完成。内容和作用域未变化时可在同一会话复用，切换仓、路径或职责时补读。
...
- 新实施计划使用 `task-evidence-v1`；验证后先写任务证据，再勾选并以 `trustedProgress` 继续。计划不复制或绑定项目规范。
