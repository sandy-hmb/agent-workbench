...
- `design/design.md` 记录完整怎么做：现状、候选方案与取舍、组件和数据流、D01-DNN 决策及理由、关键数据模型和迁移概览、主要接口和状态映射、错误处理、兼容、回退与验证。它是唯一主入口。只有独立读者、审阅或维护需要时，经确认拆 `design/data-model.md` 或 `design/api-integration.md`；附件扩展主设计，不替代主设计，并声明所扩展的 D 编号和链接回主设计。方案确认生成后创建并自审，再由使用者批准整个 Design 审阅包。新建、删除或实质修改任一附件时，设计审阅和已有计划审阅都回到“待审阅”。
- 书面设计获批后，`workspace-writing-plan` 直接创建并自审 `plans/implementation.md` 草案，不重复确认计划摘要。任务使用 `- [ ] T01`，并写明主设计 R/D 依据、落点、依赖和具体验证；需要实现细节时再附加附件章节链接。该文件记录可执行任务和跨会话读取清单，不新增 `plan.md` 或 `tasks.md`。使用者审阅实际计划、基线、分支和执行方式并明确批准后，更新 README 的计划审阅和状态为 `development`。
