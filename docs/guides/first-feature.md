...
- `design/design.md` 记录完整怎么做：现状与总体方案、必要的数据/API/状态概览、D01-DNN 决策、跨模块风险与验证、需求覆盖和附件导航。决策必须说明选择及理由，并按内容使用短段落、表格或编号步骤；覆盖矩阵放在决策之后，具体风险不在末尾重复。它是唯一主入口。只有独立读者、审阅或维护需要时，经确认拆 `design/data-model.md` 或 `design/api-integration.md`；附件扩展主设计，不替代主设计，并声明所扩展的 D 编号和链接回主设计。方案确认生成后完成内容、结构和 Markdown 阅读自审，再由使用者批准整个 Design 审阅包。新建、删除或实质修改任一附件时，设计审阅和已有计划审阅都回到“待审阅”。
- 书面设计获批后，`workspace-writing-plan` 直接创建并自审 `plans/implementation.md` 草案，不重复确认计划摘要。计划先列执行概览，再按实际业务或交付领域选择性分组；小计划不增加分组。任务使用 `- [ ] T01`，先说明交付结果，再按需列改动位置、实施步骤和独立验证；主设计 R/D 依据和显式依赖保持稳定，需要实现细节时再附加附件章节链接。长命令使用可执行的多行续行格式。该文件记录可执行任务和跨会话读取清单，不新增 `plan.md` 或 `tasks.md`。使用者审阅实际计划、基线、分支和执行方式并明确批准后，更新 README 的计划审阅和状态为 `development`。
