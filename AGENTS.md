...
默认只有 `design/design.md`；它始终是完整、自包含的技术设计和唯一主入口，保留总体方案、模块和数据流、全部规范 D01-DNN 决策及其理由、关键数据模型和迁移概览、主要接口及兼容策略、状态转换、风险和验证方式。接口、数据库或上线策略使用按需附件；附件扩展主设计，不替代或抽空主设计。复杂数据模型或迁移只有经用户确认才拆为 `design/data-model.md`，复杂接口契约只有经用户确认才拆为 `design/api-integration.md`，其他专题也必须有独立读者、审阅或维护理由并从主设计链接。D01-DNN 的规范定义只位于主设计；附件以稳定本地锚点或“D02 扩展”标题展开细节。实施计划优先引用主设计 D 编号，需要实现细节时再附加附件章节链接；计划保留一个包含任务复选框的主文件，不新增 `plan.md` 或 `tasks.md`，避免状态续接依赖多个计划文件。

Design 和 Plan 按读者需要组织 Markdown：先给总体方案或交付结果，再按内容使用表格、编号步骤、列表和独立验证区块；业务分组只在确有多个自然领域时使用。空行和换行服务于块级结构，token 优化依靠引用权威定义和删除重复正文，不以行数衡量。
