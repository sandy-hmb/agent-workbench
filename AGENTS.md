...
Requirements 和 Design 先讨论会实质改变结果的未决项；提问轮次、选项与模式降级规则由 `workspace-feature-design` 定义。结论收敛后直接写入并自审，将 README 对应审阅状态标为“待审阅”；用户批准实际文件后标为“已批准”，并在没有新阻塞时于同一轮生成下一阶段草案。当前只有一个明确审阅对象时，“确认”“ok”“可以”“好的”“批准”等无附加条件的肯定回复均可表示批准；包含修改、否定、暂停或对象不唯一时先处理歧义。设计正文与全部附件构成一个审阅包，新建、删除或实质修改任一附件时，设计审阅与已有计划审阅一并退回“待审阅”；唯一例外的定向修订规则见 `workspace-writing-plan`。历史 feature 缺少审阅状态时显示“未记录”，不据此推断批准或重置生命周期。
