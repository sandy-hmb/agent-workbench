...
Requirements 和 Design 先讨论会实质改变结果的未决项；每轮问题数量、选项和模式降级规则由 `workspace-feature-design` 定义。结论收敛后必须单独取得生成确认，未回复、含糊回复或仅要求开始工作不视为确认；写入并自审后将 README 对应审阅状态标为“待审阅”，用户批准实际文件后标为“已批准”。`design/design.md` 和全部设计附件组成一个 Design 审阅包；新建、删除或实质修改任一附件时，将 README 的设计审阅和已有计划审阅一并改为“待审阅”，待用户批准整个审阅包后才标记设计“已批准”。书面设计获批后由 `workspace-writing-plan` 直接生成计划草案，不重复确认摘要；实际计划、基线、分支和执行方式获批后才更新为 `development`。历史 feature 缺少审阅状态时显示“未记录”，不据此推断批准或重置生命周期。

Requirements 开头说明目标，正文只维护范围、可观察行为与验收、边界、假设和来源，不写技术方案。默认只有 `design/design.md`；它始终是完整、自包含的技术设计和唯一主入口，先说明目标、选定方案和关键约束，存在真实取舍时比较可行方案，再按需展开总体方案、模块和数据流、关键数据模型和迁移概览、主要接口及兼容策略、状态转换、风险和验证方式。接口、数据库或上线策略使用按需附件；附件扩展主设计，不替代或抽空主设计。复杂数据模型或迁移只有经用户确认才拆为 `design/data-model.md`，复杂接口契约只有经用户确认才拆为 `design/api-integration.md`，其他专题也必须有独立读者、审阅或维护理由并从主设计链接。只有需要稳定引用的关键决策使用 D01-DNN；D01-DNN 的规范定义只位于主设计，附件以稳定本地锚点或“D02 扩展”标题展开细节。不适用的章节直接删除。

Design 和 Plan 按读者需要组织 Markdown：先给总体方案或交付结果，再按内容使用表格、编号步骤、列表和独立验证区块；业务分组只在确有多个自然领域时使用。实施计划开头集中说明目标、方案、技术栈、设计来源、执行方式和全局约束；每个任务按交付结果、精确 Files、Interfaces、依赖、单动作步骤和验证组织。计划提交审阅前核对全部任务的实际代码位置、接口、依赖、验证命令和行为边界。实施计划优先引用主设计 D 编号，需要实现细节时再附加附件章节链接；只保留一个包含顶层任务复选框的主文件，不新增 `plan.md`、`tasks.md` 或嵌套任务状态。空行和换行服务于块级结构，token 优化依靠引用权威定义和删除重复正文，不以行数衡量。
